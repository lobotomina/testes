...
st.markdown("""
<div class="data-info">
    <h3>Dados sobre abandono escolar</h3>
    <p>Dados sobre abandono escolar e informações geográficas separados por ano. 
    Selecione o ano desejado na barra lateral esquerda para conferir os dados.</p>
</div>
""", unsafe_allow_html=True)
...
def load_data():
    ...
    try:
        ...
        df_geo = pd.read_csv("../data/municipios.csv", encoding="latin1")
...
def folium_map_to_html(folium_map):
    """Converte um mapa Folium para HTML e exibe no Streamlit"""
    # Salvar o mapa em um arquivo HTML temporário
    with tempfile.NamedTemporaryFile(delete=False, suffix='.html') as tmp_file:
        folium_map.save(tmp_file.name)
    ...
    # Exibir o mapa HTML no Streamlit
...
if data_loaded and df is not None and df_geo is not None:
    ...
    **Período:** {df['Ano'].min()} - {df['Ano'].max()}
    ...
    mostrar_mapas = st.sidebar.checkbox("Mostrar mapas interativos", value=True)
    ...
    if gdf is not None:
        ...
        tabs_list = []
        if mostrar_mapas:
            tabs_list.extend(["Mapa de Clusters", "Mapa de Significância"])
        if mostrar_graficos:
        ...
        if tabs_list:
            ...
            if mostrar_mapas:
                ...
                with tabs[tab_index]:
                    ...
                    folium_map_to_html(mapa_cluster)
...
st.markdown(
    ...
    "Trabalho da disciplina de Machine Learning - PPGTD/UFT | Desenvolvido com Streamlit"
    "</div>", 
    unsafe_allow_html=True
)